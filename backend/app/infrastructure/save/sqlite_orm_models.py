...
class LifeCurrentStateRow(SqliteBase):
...
class LifeYearSnapshotRow(SqliteBase):
    ...
    rule_version: Mapped[str] = mapped_column(String(32), nullable=False)
    state_before_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    state_after_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    year_result_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    narrative_result_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    triggered_random_events_json: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    legal_events_json: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    mainline_changes_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    achievement_changes_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    milestones_json: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    death_result_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    inheritance_result_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class LifeEventLogRow(SqliteBase):
...
class TimelineEntryRow(SqliteBase):
...
class LifeInheritanceRow(SqliteBase):
...
class LifeHeirContinuationRow(SqliteBase):
